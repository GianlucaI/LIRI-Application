...
console.log('inforEntered = ' + infoEntered);
  ...
  request(queryURL, function(error, response, body) {
      ...
      if (body) {
        ...
        if (data.Error == 'Movie not found!') {
          ...
          fs.appendFile('log.txt', logNoMovies, function(err) {
            if (err) {
              return console.log(
                'No movie by that title data did not append to log.txt file.'
              );
            }
          });
    ...
    if (error) {
      ...
      fs.appendFile('log.txt', logMovieError, function(err) {
        if (err) {
          return console.log(
            'OMDBapi response error message did not append to log.txt file.'
          );
        }
      });
    }
...
function spotifyTitle() {
  if (process.argv[3] === undefined) {
    ...
    console.log('Spotify Title Undefined: ' + title);
    spotifyInfo();
  } else if (title !== undefined) {
    //titleSplit = title.split(' ');
    //title = titleSplit.join('%20');
    console.log('Spotify Title NOT Undefined: ' + title);
    spotifyInfo();
  }
}
...
function spotifyInfo() {
  console.log('Spotify query title: ' + title);
  spotify.search(
    {
      type: 'track',
    ...
    function(err, data) {
      if (err) {
        return console.log(
          'Spotify song data was not appended to the log.txt file.' + err
        );
      }
      ...
      if (data) {
        ...
        fs.appendFile('log.txt', logNoSpotify, function(err) {
          if (err) {
            return console.log(
              'Spotify no song data found was not appended to the log.txt file.'
            );
          }
        });
      }
  ...
  client.get('statuses/user_timeline', params, function(
    ...
    if (!error) {
      ...
      fs.appendFile('log.txt', logTweetHeader + '\n', function(err) {
        if (err) {
          return console.log(
            'Twitter header was not appended to the log.txt file.'
          );
        }
      });
      for (i = 0; i < tweets.length; i++) {
        ...
        fs.appendFile(
          ...
          function(err) {
            if (err) {
              return console.log(
                'Twitter data was not appended to the log.txt file.'
              );
            }
          }
        );
  ...
  fs.readFile('random.txt', 'utf8', function(err, data) {
    if (err) {
      ...
      fs.appendFile('log.txt', logDoIt, function(err) {
        if (err) {
          return console.log(
            'do-what-it-says data was not appended to the log.txt file.'
          );
        }
      });
    }
